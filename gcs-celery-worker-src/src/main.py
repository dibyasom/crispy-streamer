...
if __name__ == '__main__':
            ...
            if success:
                if not counter:
                    ...
                    if t1-chk_pt >= WINDOW_SIZE or first_pass:
                        ...
                        dest_file_name = f"{start_stamp}.mkv"
    ...
    # For cool exit-event-message, no reason to exist.
        ...
        total_time_elapsed = round(CHK_PT_END-CHK_PT, 3)
        print("Hello world")
        print(f"# of Frames processed* : {p_counter}")
        print(f"Actual # of frames read : {p_counter}")
        print(
            f"Avg proc time /frame (buffer): {round(time_consumed_in_memory/p_counter, 5)} second(s)")
        print(f"Total time elapsed : {total_time_elapsed} second(s)")
        print(
            f"Estimated upload processing speed (Aynschronous) : {p_counter/total_time_elapsed} FPS")
